...
The **complete test suite runs only in GitHub Actions**, including on contributors' machines with Docker. Push your branch to your fork with Actions enabled, or open a pull request. [Verify Tagryn](.github/workflows/verify.yml) runs the Rust integrity tests, Node tests, Playwright checks, runtime verification, and platform bundles. Fork pull requests may require maintainer approval before GitHub starts the workflow. Packaging-only changes use [native installer checks](.github/workflows/package-preview.yml) against an already-tested release commit. Documentation-only changes do not repeat the application matrix; check their links, commands, and formatting. Raw CI bundles are temporary development artifacts; the maintainer separately publishes verified preview packages to Releases with checksums and explicit signing status.
